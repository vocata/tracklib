...
class IMMEOPFilter(EOFilterBase):
    ...
    def __init__(self,
        ...
        self._Ns = Ns
        self._Neff = Neff
        self._trans_fcn = trans_fcn
        self._models_n = len(trans_fcn)
        if self._models_n == 1:
    ...
    def init(self, state, cov, df, extension):
        ...
        self._index[:], _ = disc_random(self._probs, self._Ns)
    ...
    def predict(self):
        ...
        for i in range(self._Ns):
            ...
            self._index[i] = idx[0]
        for i in range(self._Ns):
            idx, ext = self._index[i], self._ext_samples[i]
            self._ext_samples[i] = self._trans_fcn[idx](ext)
        for i in range(self._Ns):
            mean = np.dot(self._F, self._state_samples[i])
            cov = np.kron(self._ext_samples[i], self._Q)
            self._state_samples[i] = st.multivariate_normal.rvs(mean, cov)
